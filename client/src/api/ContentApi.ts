...
export const fetchHome1ById = async (token: string, userId: string): Promise<any> => {
    try {
        const response = await fetch(`${backendEndPoint}/home1/${userId}`, {
            method: 'GET',
            headers: {
        ...
        throw new Error('Failed to fetch Home1 by ID'); 
    }
};
